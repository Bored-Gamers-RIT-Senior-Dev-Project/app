...
const MatchesPage = () => {
  ...
  useEffect(() => {
    const fetchMatches = async () => {
      try {
        const res = await fetch(`http://localhost:3000/api/tournaments/${id}/matches`);
        const data = await res.json();
        if (data) {
          setTournament(data);
          setFilters((prev) => ({ ...prev, tournamentName: data.tournamentName }));
        }
      } catch (err) {
          ...
          <Grid item xs={12} sm={6}>
            <LocalizationProvider dateAdapter={AdapterDateFns}>
              <DatePicker
                ...
                onChange={(newDate) =>
                  setFilters((prev) => ({ ...prev, dateRange: [newDate, prev.dateRange[1]] }))
                }
          ...
          <Grid item xs={12} sm={6}>
            <LocalizationProvider dateAdapter={AdapterDateFns}>
              <DatePicker
                ...
                onChange={(newDate) =>
                  setFilters((prev) => ({ ...prev, dateRange: [prev.dateRange[0], newDate] }))
                }
                renderInput={(params) => <TextField {...params} fullWidth />}
              />
            </LocalizationProvider>
